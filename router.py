...
ACTION_RE = re.compile(r"\b(?:kerjakan|selesaikan|lanjutkan|lanjut|fix|perbaiki|debug|ubah|edit|implement|test|uji|cek\s+repo|inspect\s+repo|audit)\b", re.I)
CREATE_RE = re.compile(r"\\bbuat\\s+(?:file|branch|commit|pull request|pr|endpoint|api|fungsi|function|script|kode|code|folder|service|deployment)\\b", re.I)\nSTATUS_RE = re.compile(r"\b(?:status|checkpoint|progress|progres|pending|roadmap|sampai\s+mana|posisi\s+terakhir)\b", re.I)
